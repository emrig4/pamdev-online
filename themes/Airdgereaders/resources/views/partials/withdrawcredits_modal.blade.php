        ...
        <div  class="relative inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <form action="{{ route('wallet.withdraw') }}", method="post">
                    ...
                    <div class="">
                        <div class="flex justify-between items-center space-x-5">
                            <div class="flex items-center space-x-5">
                                ...
                                <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Create Withdraw Request </h3>
                            </div>
                        ...
                        <div class="mt-8 text-center sm:ml-4 sm:text-left">
                            ...
                            <div class=" w-full mt-4">
                                <div class=" w-full">
                                    ...
                                    <input type="number" id="ranc_amount" class="w-full" name="ranc_amount" style="min-height: 40px;">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    <button type="submit"  class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-gray-600 text-base font-medium text-white hover:bg-gray-400  sm:ml-3 sm:w-auto sm:text-sm">Create</button>
                </div>
            </form>
        </div>
    </div>
...
@push('js')
    <script>
        ...
        function createWithdrawalRequest(){
            let ranc_amount =  Number (document.getElementById('ranc_amount').value)
            let email  = '<?php echo  auth()->user()->email ; ?>'
          createRequest(ranc_amount, email);
        }
        ...
        function createRequest(payload){
            ...
            http.onload = (res) => {
                 ...
                 if (http.status === 201) {
                        ...
                        var msg = '<b>Success:</b> The subscription reference is ' + data.data;
